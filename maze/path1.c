//Only the blank cell center is considered as the path point, perfect for maze.
...
void A_Star_calc(AStarDataPack *data, int n)
{
	...
	if (data->status == 1) {
		...
		data->path[0] = goal;
		for (int i = 1; i < data->plen; ++i) {
			int pr = *MGet(pre_r, data->path[i-1].r, data->path[i-1].c);
			int pc = *MGet(pre_c, data->path[i-1].r, data->path[i-1].c);
			vec2 prev = {pr, pc};
			data->path[i] = prev;
		}
